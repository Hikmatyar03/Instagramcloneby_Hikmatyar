    ...
    uploadMessageMedia(req, res, async (err) => {
        ...
        try {
            ...
            const result = await uploadBuffer(req.file.buffer, {
                folder: `instaclone/messages/${req.user._id.toString()}`,
                resource_type: isVideo ? 'video' : 'image',
            });

            const msg = await MessageService.sendMessage(req.params.id, req.user._id, {
                message_type: req.body.message_type || 'media',
